...
# MES 配置
MES_URL = "http://8.152.101.17:9091"
MES_USERNAME = "admin"
...
class BatchProcessor:
    ...
    def process_batch(self, files_list, progress_callback=None):
        ...
        for idx, uploaded_file in enumerate(files_list[:self.max_batch_size]):
            # 用 文件名+序号+时间戳后缀 确保唯一（同名文件也能分别检测）
            import hashlib
...
with st.sidebar:
    st.markdown("### 🔗 MES 集成")
    mes_url_input = st.text_input("MES 服务器地址", value="非管理员不可见")
...
with col2:
    st.markdown(
        ...
        <p style="text-align: center; font-size: 1rem; color: #4a627a; margin-top: 0;">基于 YOLO26 的缺陷检测系统 —— MES团队</p>
        """,
        unsafe_allow_html=True
    )
    ...
    for result in results:
        ...
        if file_key not in st.session_state.detection_cache:
            ...
            if st.session_state.auto_upload_enabled and file_key not in st.session_state.auto_uploaded_files:
                ...
                print(f"   🚀 正在发送到MES: {file_key}, base64长度(原图)={len(image_base64) if image_base64 else 0}, base64长度(检测图)={len(detection_image_base64) if detection_image_base64 else 0}")
                ...
                if send_success:
                    ...
                    st.toast(f"✅ {file_key} 已同步到MES", icon="✅")
                else:
                    print(f"   ❌ 自动上传失败: {file_key}")
                    st.error(f"⚠️ {file_key} 发送到MES失败，请检查网络或MES服务状态")
            else:
                if not st.session_state.auto_upload_enabled:
                    print(f"   ⏭️ 自动上传已禁用，跳过: {file_key}")
                elif file_key in st.session_state.auto_uploaded_files:
                    print(f"   ⏭️ 已上传过，跳过: {file_key}")
